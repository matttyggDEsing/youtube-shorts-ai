 ...
 */
export async function runPipeline({ category, voice, autoUpload = false, onProgress = null, precomputedStory = null }) {
  const id      = uuidv4();
  ...
  try {
    // ── PASO 1: Generar historia con Groq (o usar una ya generada por tendencia) ──
    emit('story', 10, 'Generando historia con IA...');
    const story = precomputedStory || await generateStory(category, 55);
